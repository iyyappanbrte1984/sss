  ...
  try {
    ...
    if (!GEMINI_API_KEY || !SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
      return res.status(500).json({
        error: 'Missing env vars (GEMINI_API_KEY / SUPABASE_URL / SUPABASE_SERVICE_KEY)'
      });
    }
    ...
    // QUOTA check (now provider = 'gemini')
...
Provide a short (2–4 sentence) assessment and simple suggested actions if needed. Use clear, student‑friendly language.`;
    ...
    // Store prediction (provider now 'gemini')
    const storePayload = {
      sample_id: latest.id ?? null,
      provider: 'gemini',
      model: gemJson?.model ?? 'gemini-1.5-flash-latest',
      score: null,
